...
resource "oci_ocvp_sddc" "test_sddc" {
  // Required
  compartment_id              = var.compartment_ocid
  compute_availability_domain = var.region
  esxi_hosts_count            = "3"
  ...
  ssh_authorized_keys         = file(var.public_key_oci) //"ssh-rsa  MustWin"
  vmotion_vlan_id             = oci_core_vlan.test_vmotion_net_vlan.id
  vmware_software_version     = lookup(data.oci_ocvp_supported_vmware_software_versions.test_supported_vmware_software_versions.items[0], "version")
